...
mfpc_rotation <- function(eta_in, time_g, C_g, Psi_g = NULL) {
...
foo <- function(eta_in, time_g, C_g, Psi_g) {
  ...
  zeta_rotn <- tcrossprod(D_psi, E_q_Zeta %*% V_psi)
  m_zeta <- apply(zeta_rotn, 1, mean)
  zeta_shift <- zeta_rotn - tcrossprod(m_zeta, rep(1, N))
  C_zeta <- cov(t(zeta_shift))
  eigen_C <- eigen(C_zeta)
  Q_zeta <- eigen_C$vectors
  Lambda_zeta <- diag(eigen_C$values + 1e-10)
  sqrt_C <- Q_zeta %*% sqrt(Lambda_zeta)
  tinv_sqrt_C <- Q_zeta %*% solve(sqrt(Lambda_zeta))

  Psi_tilde <- U_psi %*% sqrt_C
  Zeta_tilde <- crossprod(zeta_shift, tinv_sqrt_C)

  mu_hat <- E_q_mu + as.vector(U_psi %*% m_zeta)
  Psi_hat <- matrix(NA, p*n_g, L)
  Zeta_hat <- matrix(NA, N, L)
  norm_vec <- rep(NA, L)
  ...
  mu_hat <- split(mu_hat, rep(1:p, each = n_g))
  Psi_hat <- lapply(split(Psi_hat, rep(1:p, each = n_g)), matrix, nrow = n_g, ncol = L)

  Cov_zeta_hat <- vector("list", length = N)
  rotn_mat <- V_psi %*% D_psi %*% tinv_sqrt_C %*% diag(norm_vec)
  for(i in 1:N) {
  ...
  for(i in 1:N) {
    ...
    Y_hat <- matrix(NA, n_g, p)
    Y_low <- matrix(NA, n_g, p)
    Y_upp <- matrix(NA, n_g, p)
    for(j in 1:p) {

      sd_vec <- sqrt(diag(tcrossprod(Psi_hat[[j]] %*% Cov_zeta_hat[[i]], Psi_hat[[j]])))

      Y_hat[,j] <- mu_hat[[j]] + Psi_hat[[j]] %*% Zeta_hat[i, ]
      Y_low[,j] <- Y_hat[,j] + qnorm(0.025)*sd_vec
      Y_upp[,j] <- Y_hat[,j] + qnorm(0.975)*sd_vec
    }
    Y_summary[[i]] <- list(Y_hat, Y_low, Y_upp)
    names(Y_summary[[i]]) <- c("Y_hat", "Y_low", "Y_upp")
  }
  ...
  gbl_summary <- vector("list", length = L + 1)
  gbl_summary[[1]] <- as.matrix(Reduce(cbind, mu_hat)) # added as.matrix to account for case p = 1
  for(l in 1:L) {

    gbl_summary[[l+1]] <- matrix(NA, n_g, p)
    for(j in 1:p) {

      gbl_summary[[l+1]][, j] <- Psi_hat[[j]][, l]
    }
  }
...
mlfpc_rotation <- function(
  ...
  for(i in 1:n_sample) {
    ...
    for(j in 1:m_sample) {
      ...
      sd_vec <- sqrt(diag(Cov_11 + Cov_22 + Cov_12 + Cov_21))
