...
if (run_model_choice_version) {
  ...
  if (model_choice == "K") {
    ...
    fpca_res_mc <- run_mfvb_fpca_model_choice(time_obs, Y,
  ...
  Psi_hat_mc <- fpca_res_mc$list_Psi_hat
  ...
  for (l in 1:(L+1)) {
    ...
    if (l == 1) {
      ise_mfpca[l] <- trapint(time_g, (mu_hat - mu_g)^2)
      ise_mfpca_mc[l] <- trapint(time_g, (mu_hat_mc - mu_g)^2)
    } else {
      ise_mfpca[l] <- trapint(time_g, (Psi_hat[, l-1] - Psi_g[,l-1])^2)
      ise_mfpca_mc[l] <- trapint(time_g, (Psi_hat_mc[, l-1] - Psi_g[,l-1])^2)
    }
